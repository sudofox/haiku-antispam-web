...
// Test for graphing Bayesian results on any Hatena ID from Haiku

// Graph type: Basic Histogram
...
//$data["query"]["entries"] = $db->query("SELECT * from antispam_unclassified where timestamp > (strftime('%s', 'now')-86400) and hatena_id like '$hatena_id'");
...
while ($row = $data["query"]["entries"]->fetchArray()) {

/*
	$data["plot"][$row["hatena_id"]]["entries"][] = array("score" => $row["spam_check_score"], "details" => $row["spam_check_score_details"]);
	$data["plot"][$row["hatena_id"]]["by_score"][$row["spam_check_score"]]++;
*/
	$data["plot"][] = $row["spam_check_score"];
	if ($row["spam_check_score"] < $data["minScore"]) { $data["minScore"] = $row["spam_check_score"]; }
	if ($row["spam_check_score"] > $data["maxScore"]) { $data["maxScore"] = $row["spam_check_score"]; }
	$data["details"][] = $row["spam_check_score_details"];
}
...
<head>
    ...
    <title>Hatena Haiku Anti-Spam -- id:<?php echo $_GET["hatena_id"]; ?></title>
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <meta property="og:title" content="Spam Report for id:<?php echo $_GET["hatena_id"]; ?>"/>
    <meta property="og:description" content="Here is the spam check for id:<?php echo $_GET["hatena_id"]; ?> (http://profile.hatena.ne.jp/<?php echo $_GET["hatena_id"]; ?>). Check here for classification details, samples, and the score distribution chart.">
...
function renderSpamPreview(itemID, base64Body) {

	var c = document.getElementById("spam-preview-" + itemID);
	var ctx = c.getContext("2d");
ctx.font = "15px monospace";ctx.font = "15px monospace";
	var spamText = b64DecodeUnicode(base64Body).replace(/(\r\n\t|\n|\r\t)/gm,"");
	if (spamText.length < 55) {
		ctx.fillText(spamText,0,20);
	} else {
        	split_length = 55;
        	var lengthRegex = new RegExp(".{" + split_length + "}","g");
		spamTextSplit = spamText.match(lengthRegex);
		if (spamTextSplit == null) {
			/* Temp fix */
			console.log("Temp fix -- could not match length even though string should've been long enough (renderSpamPreview)");
			ctx.fillText(spamText,0,20);
			return;
		}
		console.log(spamTextSplit);
		var remainder = spamText.length-(spamTextSplit.length*split_length);
		for (var k in spamTextSplit){

			if (spamTextSplit.hasOwnProperty(k)) {
			ctx.fillText(spamTextSplit[k],0,(19*k)+20);
	    	}
	}

	if (remainder > 0 ) {
		ctx.fillText(spamText.substr(spamTextSplit.length*split_length, remainder), 0, spamTextSplit.length*19+20);
	}
}
}
</script>
...
<!--          <li class="nav__item"><a href="https://lightni.ng">Lookup by Hatena ID</a></li> -->
        </ul>
      </nav>
    </header>
    <section class="content wrap">
      <article class="post">
        <div class="post__body">
         <h5>Recent Spam Scores <!--(24 Hours)--> for id:<?php echo $hatena_id; ?></h5>
         <p>Any score above five is considered spam.</p>
	 <p style="margin: 0;">Sampling <?php echo $data["query"]["summary"]["count"]; ?> of id:<?php echo $hatena_id; ?>'s posts, I found a spam score average of <?php echo round($data["query"]["summary"]["average_score"],2); ?> (<?php echo $data["query"]["summary"]["marked_spam"] ." spam, " . $data["query"]["summary"]["marked_ham"] . " not spam"; ?>).</p>
	 <p>(I believe this user is <span style="<?php echo ($data["query"]["summary"]["average_score"] > 5) ? 'color:red;"> likely to post spam' : 'color:green;"> unlikely to post spam'; ?></span>)</p>
  ...
  <script>
var scores = <?php echo json_encode($data["plot"]); ?>;
...
var layout = {
  xaxis: { range: [-20, <?php echo $data["maxScore"]; ?>], title: "Score (lower is better)" },
...
<div class="spam-output">
<h4 style="margin-left:10px; margin-top:5px;">Example Posts</h3>
<div class="spam-output-scrollable">
<?php while ($row = $data["query"]["entries"]->fetchArray()) {
	$associativeData[] = $row;
}
foreach (array_slice($associativeData,0,10) as $row) {
//        $data["plot"][] = $row["spam_check_score"];
//        $data["details"][] = $row["spam_check_score_details"];
?><div class="bayes-entry" style="background-color: #f4f5f4; box-shadow: 0px 3px 10px -5px black;">
<div class="bayes-header"><h5>Entry ID <?php echo $row["post_id"]; ?></h5>
<canvas id="spam-preview-<?php echo $row["post_id"]; ?>" height="100" width="500" style="border:1px solid #d3d3d3; display: block; margin: 0 auto;"></canvas>
<script>renderSpamPreview("<?php echo $row["post_id"]; ?>", "<?php echo base64_encode($row["body"]); ?>");</script>
<br>
Detected as <?php

if ($row["spam_check_judgement"]) {
	?><span style="color:red;">spam</span><?php
	} else {
	?><span style="color:green;">not spam</span><?php
	}
?> with a score of <b><tt><?php echo $row["spam_check_score"] ."/". $row["spam_check_threshold"]; ?></tt></b>. Factors involved:</div>
<pre>
<code class="language-"><?php echo $row["spam_check_score_details"]; ?></code></pre></div>
<?php } ?>
</div>
</div>
<br><br>
